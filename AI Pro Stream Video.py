...
shared_labels = []             
shared_fps = 0.0               
shared_realtime_fps = 0.0
shared_display_fps = 0.0
shared_frame_size = "--"
shared_device_label = "GPU"
...
CAMERA_SOURCE = os.environ.get("CAMERA_SOURCE", "raw://192.168.31.177:5001")
SERVER_PORT = int(os.environ.get("SERVER_PORT", "5000"))

# 模型控制
DEFAULT_YAML = "GC10.yaml"
CURRENT_MODEL = os.environ.get("MODEL_PATH", "./last.pt")
TARGET_MODEL = None   
INFER_ON = True       
...
def raw_stream_endpoint(source):
    parsed = urlparse(str(source).strip())
    if parsed.scheme != "raw" or not parsed.hostname:
        return None
    return parsed.hostname, parsed.port or 5001
...
def recv_exact(sock, size):
    ...
    while len(data) < size:
        ...
        if not chunk:
            raise ConnectionError("raw stream disconnected")
        data.extend(chunk)
    return bytes(data)

def raw_frame_reader(source):
    endpoint = raw_stream_endpoint(source)
    if endpoint is None:
        return
        ...
        try:
            with socket.create_connection((host, port), timeout=5.0) as sock:
                ...
                while True:
                    ...
                    magic, _ts_ns, width, height, channels, payload_size = struct.unpack(header_fmt, header)
                    if magic != b"RAW0":
                        raise ValueError("收到的不是 RAW0 原始视频流")
                    payload = recv_exact(sock, payload_size)
                    expected = width * height * channels
                    if payload_size != expected:
                        raise ValueError(f"原始帧大小异常: {payload_size} != {expected}")
...
class GPU_Detector:
    ...
    def load_yaml(self, model_path):
        ...
        if os.path.exists(yaml_path_direct):
            ...
            print(f"未找到对应的 YAML 标签文件，回退使用 {DEFAULT_YAML}")
    ...
    def load_model(self, model_path):
        global shared_device_label

        ext = os.path.splitext(model_path)[1].lower()
        if ext not in (".pt", ".onnx", ".engine"):
            raise RuntimeError("电脑GPU端仅支持 .pt/.onnx/.engine 模型，请先使用可在电脑端推理的权重。")

        try:
            from ultralytics import YOLO
            import torch
        except Exception as e:
            raise RuntimeError(f"GPU推理需要安装 ultralytics 和 torch: {e}")

        device = GPU_DEVICE
        if device.startswith("cuda") and not torch.cuda.is_available():
            print("未检测到可用 CUDA，自动切换到 CPU 推理。")
            device = "cpu"

        self.model = YOLO(model_path)
        self.device = device
        shared_device_label = "GPU" if device != "cpu" else "CPU"

        names = getattr(self.model, "names", {})
        if isinstance(names, dict) and names:
            self.class_names = {int(k): str(v) for k, v in names.items()}
        else:
            self.load_yaml(model_path)

        print(f"模型加载完成: {model_path}, device={device}")
    ...
    def postprocess_gpu(self, result, scale):
        boxes_obj = getattr(result, "boxes", None)
        if boxes_obj is None or len(boxes_obj) == 0:
            return [], [], []

        boxes = boxes_obj.xyxy.detach().cpu().numpy() * scale
        scores = boxes_obj.conf.detach().cpu().numpy()
        class_ids = boxes_obj.cls.detach().cpu().numpy().astype(int)
        return boxes.tolist(), scores.tolist(), class_ids.tolist()

    def infer_once(self, frame):
        scale, canvas = self.preprocess(frame)
        ...
        return self.postprocess_gpu(results[0], scale)

    def camera_worker(self):
        global latest_clean_frame, global_frame, shared_fps, shared_realtime_fps, shared_frame_size
        global shared_boxes, shared_scores, shared_labels

        prev_frame_time = time.time()

        def handle_frame(frame):
            nonlocal prev_frame_time
            global latest_clean_frame, global_frame, shared_realtime_fps, shared_frame_size

            now = time.time()
            realtime_fps = 1.0 / (now - prev_frame_time + 1e-6)
            prev_frame_time = now
            shared_realtime_fps = shared_realtime_fps * 0.9 + realtime_fps * 0.1
            h, w = frame.shape[:2]
            shared_frame_size = f"{w} x {h}"
            ...
            status_text = f"INFER FPS: {shared_fps:.1f}" if INFER_ON else f"{shared_device_label} STATUS: PAUSED"
            color = (0, 255, 0) if INFER_ON else (0, 0, 255)
            cv2.putText(frame, status_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, color, 3)
            cv2.putText(frame, f"TRANSFER FPS: {shared_realtime_fps:.1f}", (20, 95),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 0), 2)
            global_frame = frame

        if raw_stream_endpoint(CAMERA_SOURCE):
            for frame in raw_frame_reader(CAMERA_SOURCE):
                handle_frame(frame)
            return
        ...
        while True:
            ...
            if not ret:
                fail_count += 1
                if fail_count % 100 == 0:
                    print(f"暂未读取到视频帧，请检查视频源: {CAMERA_SOURCE}")
                time.sleep(0.03)
                continue
    ...
    def infer_worker(self):
        ...
        while True:
            ...
            if not INFER_ON or not model_ready:
                shared_boxes, shared_scores, shared_labels = [], [], []
                shared_fps = 0.0
                time.sleep(0.1)
                continue
            ...
            try:
                shared_boxes, shared_scores, shared_labels = self.infer_once(frame)
            except Exception as e:
...
def get_models():
    return jsonify({"models": available_model_files(), "current": CURRENT_MODEL})
...
def sys_info():
    info = {
        "transfer_fps": f"{shared_realtime_fps:.1f}",
        "transfer_size": shared_frame_size,
        "infer_speed": f"{shared_fps:.1f}",
        "realtime_speed": f"{shared_display_fps:.1f}"
    }
    return jsonify(info)
...
def generate_video_stream():
    global global_frame, shared_display_fps
    last_sent_id = 0
    ...
    while True:
        ...
        now = time.time()
        display_fps = 1.0 / (now - prev_sent_time + 1e-6)
        prev_sent_time = now
        shared_display_fps = shared_display_fps * 0.9 + display_fps * 0.1
    ...
    <html>
        <head>
            ...
            <style>
                ...
                .status-bar { display: grid; grid-template-columns: repeat(auto-fit, minmax(115px, 1fr)); gap: 15px; }
                ...
                <div class="video-section">
                    ...
                    <div class="status-bar">
                        <div class="status-card">
                            <div class="status-label">传输帧率</div>
                            <div id="transferFps" class="status-value">--</div>
                        </div>
                        <div class="status-card">
                            <div class="status-label">传输尺寸</div>
                            <div id="transferSize" class="status-value" style="color: #FF9800;">--</div>
                        </div>
                        <div class="status-card">
                            <div class="status-label">推理速度</div>
                            <div id="inferSpeed" class="status-value">--</div>
                        </div>
                        <div class="status-card">
                            <div class="status-label">实时速度</div>
                            <div id="realtimeSpeed" class="status-value">--</div>
                        </div>
                    </div>
                </div>
            ...
            <script>
                ...
                setInterval(() => {
                    fetch('/api/sys_info').then(res => res.json()).then(data => {
                        document.getElementById('transferFps').innerText = data.transfer_fps;
                        document.getElementById('transferSize').innerText = data.transfer_size;
                        document.getElementById('inferSpeed').innerText = data.infer_speed;
                        document.getElementById('realtimeSpeed').innerText = data.realtime_speed;
                    }).catch(e => {});
                }, 2000);
            </script>
        </body>
...
def apply_runtime_args():
    global CAMERA_SOURCE, CURRENT_MODEL, GPU_DEVICE, SERVER_PORT, INPUT_SIZE

    parser = argparse.ArgumentParser(description="GPU industrial detection viewer")
    parser.add_argument("--source", default=CAMERA_SOURCE, help="摄像头源，支持raw://、本机编号、HTTP/MJPEG或RTSP地址")
    parser.add_argument("--model", default=CURRENT_MODEL, help="模型路径，支持.pt/.onnx/.engine")
    parser.add_argument("--gpu-device", default=GPU_DEVICE, help="GPU设备，例如 cuda:0")
    parser.add_argument("--port", type=int, default=SERVER_PORT, help="Web服务端口")
    parser.add_argument("--imgsz", type=int, default=INPUT_SIZE, help="模型输入尺寸")
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("GPU 工业质检台启动完毕！")
    print(f"视频源: {CAMERA_SOURCE}")
    print(f"模型: {CURRENT_MODEL} | GPU设备: {GPU_DEVICE}")
    print(f"请在浏览器中访问: http://<本机IP地址>:{SERVER_PORT}")
    print("="*60 + "\n")
