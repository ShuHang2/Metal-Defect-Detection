...
def bbox_iou_wiou(box1, box2, eps=1e-7):
    ...
    inter = (torch.min(x1 + w1 / 2, x2 + w2 / 2) - torch.max(x1 - w1 / 2, x2 - w2 / 2)).clamp(0) * \
            (torch.min(y1 + h1 / 2, y2 + h2 / 2) - torch.max(y1 - h1 / 2, y2 - h2 / 2)).clamp(0)
    union = w1 * h1 + w2 * h2 - inter + eps
    iou = inter / union

    cw = torch.max(x1 + w1 / 2, x2 + w2 / 2) - torch.min(x1 - w1 / 2, x2 - w2 / 2)
    ch = torch.max(y1 + h1 / 2, y2 + h2 / 2) - torch.min(y1 - h1 / 2, y2 - h2 / 2)
    dist = (x1 - x2) ** 2 + (y1 - y2) ** 2
    
    r_wiou = torch.exp(dist / (cw ** 2 + ch ** 2 + eps)) 

    with torch.no_grad():
        iou_loss = 1.0 - iou
        beta = iou_loss / (iou_loss.mean() + eps)
        alpha = beta / (1.9 ** (beta - 3.0))
        
    return 1.0 - (alpha * r_wiou * (1.0 - iou))


def bbox_iou_inner_wiou(box1, box2, ratio=0.75, eps=1e-7):
    ...
    inter = (torch.min(x1 + w1_inner / 2, x2 + w2_inner / 2) - torch.max(x1 - w1_inner / 2, x2 - w2_inner / 2)).clamp(0) * \
            (torch.min(y1 + h1_inner / 2, y2 + h2_inner / 2) - torch.max(y1 - h1_inner / 2, y2 - h2_inner / 2)).clamp(0)
    union = w1_inner * h1_inner + w2_inner * h2_inner - inter + eps
    ...
    with torch.no_grad():
        iou_loss = 1.0 - iou_inner
        beta = iou_loss / (iou_loss.mean() + eps)
        alpha = beta / (1.9 ** (beta - 3.0))
        
    return 1.0 - (alpha * r_wiou * (1.0 - iou_inner))

def bbox_iou_inner_wiou_dynamic(box1, box2, eps=1e-7):
    """
    box1, box2: [x_center, y_center, w, h], shape: [N, 4]
    动态 ratio 根据原始 IoU 自适应调整：
        高 IoU → ratio < 1, 低 IoU → ratio > 1
    """
    (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)

    # ------ 1. 计算原始 IoU（不缩放），用于判断样本质量 ------
    # 原始框的半宽半高
    half_w1, half_h1 = w1 / 2, h1 / 2
    half_w2, half_h2 = w2 / 2, h2 / 2

    inter_x1 = torch.max(x1 - half_w1, x2 - half_w2)
    inter_y1 = torch.max(y1 - half_h1, y2 - half_h2)
    inter_x2 = torch.min(x1 + half_w1, x2 + half_w2)
    inter_y2 = torch.min(y1 + half_h1, y2 + half_h2)

    inter_w = (inter_x2 - inter_x1).clamp(0)
    inter_h = (inter_y2 - inter_y1).clamp(0)
    inter_original = inter_w * inter_h

    area1 = w1 * h1
    area2 = w2 * h2
    union_original = area1 + area2 - inter_original + eps
    iou_original = inter_original / union_original   # shape [N,1]

    # ------ 2. 根据原始 IoU 生成动态 ratio ------
    # 映射：ratio = 1.5 - iou_original，范围 [0.5, 1.5]
    ratio = 1.5 - iou_original
    ratio = torch.clamp(ratio, 0.75, 1.25)  # 防止极端值

    # ------ 3. 用动态 ratio 生成辅助框并计算 Inner-IoU ------
    w1_inner = w1 * ratio
    h1_inner = h1 * ratio
    w2_inner = w2 * ratio
    h2_inner = h2 * ratio

    inter = (torch.min(x1 + w1_inner / 2, x2 + w2_inner / 2) -
             torch.max(x1 - w1_inner / 2, x2 - w2_inner / 2)).clamp(0) * \
            (torch.min(y1 + h1_inner / 2, y2 + h2_inner / 2) -
             torch.max(y1 - h1_inner / 2, y2 - h2_inner / 2)).clamp(0)

    union = w1_inner * h1_inner + w2_inner * h2_inner - inter + eps
    iou_inner = inter / union

    # ------ 4. 后续 WIoU 风格的计算（保持原样）------
    cw = torch.max(x1 + w1 / 2, x2 + w2 / 2) - torch.min(x1 - w1 / 2, x2 - w2 / 2)
    ch = torch.max(y1 + h1 / 2, y2 + h2 / 2) - torch.min(y1 - h1 / 2, y2 - h2 / 2)
    dist = (x1 - x2) ** 2 + (y1 - y2) ** 2
...
def bbox_iou_cw_wiou(box1, box2, cls_weight=None, eps=1e-7):
    ...
    with torch.no_grad():
        ...
        if cls_weight is not None:
            cls_weight_clamped = torch.clamp(cls_weight,max=1.5)
            alpha = alpha * cls_weight_clamped
