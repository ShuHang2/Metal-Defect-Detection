...
from tqdm import tqdm  # 用于显示扫描进度


from .metrics import bbox_iou_wiou
from .metrics import bbox_iou_cw_wiou
from .metrics import bbox_iou_inner_wiou
from .metrics import bbox_iou_inner_wiou_dynamic
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        # -----------w-iou---------------
        iou = bbox_iou_wiou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        
        # -----------inner_wiou---------------
        # iou = bbox_iou_inner_wiou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        # iou = bbox_iou_inner_wiou_dynamic(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        # -----------cwiou--------------
        # target_cls_ids = target_scores[fg_mask].argmax(-1)
        # batch_cls_weights = self.cls_weights_list[target_cls_ids].unsqueeze(-1) 
        # iou = bbox_iou_cw_wiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], cls_weight=batch_cls_weights)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        # ==========================================
        # GGW-DND: 给模型装上“梯度记忆”模块 (新增区域开始)
        # ==========================================
        # 1. 记录每个类别的累计正/负梯度 (直接放到 device 上)
        self.cumulative_pos_grad = torch.ones(self.nc, device=device) * 1e-5
        self.cumulative_neg_grad = torch.ones(self.nc, device=device) * 1e-5
        
        # 2. 准备两个专门用来存放算出来的动态权重的变量 (直接放到 device 上)
        self.w_pos = torch.ones(self.nc, device=device)
        self.w_neg = torch.ones(self.nc, device=device)

        # 3. 论文作者做过成百上千次实验得出的黄金超参数组合
        # self.alpha = 8.0         # 负梯度缩小强度的控制因子
        # self.beta = 8.0          # 正梯度放大强度的控制因子
        # self.gamma = 0.7         # 映射函数的平移阈值
        # self.decay_lambda = 6.0  # 最终强制砍掉负梯度的倍数
        
        self.alpha = 4.0         # 从 8.0 降到 4.0：减小对少数类的“过度奖励”，提升精确率
        self.beta = 8.0          # 保持 8.0：维持映射函数的灵敏度
        self.gamma = 0.8         # 从 0.7 升到 0.8：提高门槛，只有更稀有的类才触发大幅加权
        self.decay_lambda = 2.0
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        # # ==========================================
        # # Cls loss (完全复刻 GGW-DND 动态梯度引导机制)
        # # ==========================================
        # # 1. 计算当前预测的概率值 (Sigmoid 激活)
        # pred_probs = pred_scores.sigmoid()
        
        # # 2. 开启无梯度模式来记录历史状态，不影响反向传播
        # with torch.no_grad():
        #     # 区分前景(正样本)和背景(负样本)
        #     pos_mask = (target_scores > 0).float()
        #     neg_mask = (target_scores == 0).float()
            
        #     # 估算当前批次产生的正负梯度
        #     current_pos_grad = (1.0 - pred_probs) * pos_mask
        #     current_neg_grad = pred_probs * neg_mask
            
        #     # 汇总当前批次各类别产生的梯度总和 (保留类别维度)
        #     batch_pos_grad_sum = current_pos_grad.sum(dim=(0, 1)) 
        #     batch_neg_grad_sum = current_neg_grad.sum(dim=(0, 1)) 

        #     # 将当前梯度累加到历史记录中 (论文式5的核心)
        #     self.cumulative_pos_grad += batch_pos_grad_sum
        #     self.cumulative_neg_grad += batch_neg_grad_sum

        #     # 计算累计梯度比值 R_T
        #     R_T = self.cumulative_pos_grad / self.cumulative_neg_grad
        #     # 设置上限，防止前期数值不稳定导致梯度爆炸
        #     R_T_clamped = torch.clamp(R_T, max=10.0) 
            
        #     # 使用非线性映射公式更新动态权重 (论文式2和式3)
        #     self.w_pos = 1.0 / (1.0 + torch.exp(-self.beta * (R_T_clamped - self.gamma)))
        #     self.w_neg = 1.0 + self.alpha * (1.0 - self.w_pos)

        # # 3. 将一维的动态权重转换为三维 (1, 1, 类别数)，以匹配损失矩阵的形状
        # w_pos_b = self.w_pos.view(1, 1, self.nc)
        # w_neg_b = self.w_neg.view(1, 1, self.nc)

        # # 4. 计算基础的二分类交叉熵损失 (此时保留矩阵形状，不求和)
        # base_bce_loss = self.bce(pred_scores, target_scores.to(dtype))

        # # 5. 执行非对称惩罚与奖励 (论文的核心创新)
        # # 【正类损失】：对真实缺陷施加动态放大的权重 w_pos
        # pos_loss = base_bce_loss * pos_mask * w_pos_b
        
        # # 【负类损失】：对纯背景施加动态缩小的权重 w_neg，并强制除以 decay_lambda 进行延迟衰减
        # neg_loss = base_bce_loss * neg_mask * w_neg_b * (1.0 / self.decay_lambda)

        # # 6. 合并正负损失，进行全局求和归一化
        # loss[1] = (pos_loss + neg_loss).sum() / target_scores_sum
        # # ==========================================
